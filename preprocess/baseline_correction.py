...
class MSIPreprocessor():
    ...
    def baseline_correction(
        ...
        epsilon: float = 1e-3
    ) -> Union[np.ndarray, SpectrumBaseModule]:
        """
        ...
        def asls_baseline(y: np.ndarray, lam: float, p: float, niter: int) -> np.ndarray:
            ...
            try:
                ...
                # Second derivative matrix for smoothness constraint
                ...
                for _ in range(int(max(1, niter))):
                    ...
                    # Update asymmetric weights: lower weight for peaks (y > baseline)
                    w = p * (y > baseline) + (1.0 - p) * (y <= baseline)
                
                return np.asarray(baseline, dtype=np.float64)
                
            except ImportError:
                # Fallback to dense matrix implementation (slower but works without scipy.sparse)
        ...
        def snip_baseline(y: np.ndarray,
            ...
            for pval in p_iter:
                ...
                # The core of SNIP: y_k+1 = min(y_k, (y_k-p + y_k+p)/2)
                ...
                # Adaptive early stop on relative change
        ...
        def apply_single(intensity: np.ndarray):
            ...
            # Apply scaling to avoid over-correction
